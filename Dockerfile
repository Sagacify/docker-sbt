# SBT image for testing/building scalajs projects
...
RUN set -x && apk add --no-cache curl make gcc g++ python linux-headers paxctl libgcc libstdc++ gnupg bash git && \
  ...
  rm -rf /etc/ssl /node-${NODE_VERSION}.tar.gz \
    /SHASUMS256.txt.asc /node-${NODE_VERSION} ${RM_DIRS} \
